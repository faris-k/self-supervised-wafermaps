...
# and `pip install ssl_wafermap` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "ssl_wafermap", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n ssl_wafermap -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "ssl_wafermap", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "ssl_wafermap", "YOUR_CONSOLE_SCRIPT"]
